...
from megatron.core.datasets.utils import should_build_on_rank, normalize
...
class BlendedDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        if len(datasets) == 1:
            log_single_rank(logger, logging.WARNING, f"Building a BlendedDataset for a single MegatronDataset")
    ...
    def _build_indices(self) -> Tuple[numpy.ndarray, numpy.ndarray]:
        ...
        if not path_to_cache or (
            not cache_hit and should_build_on_rank(torch.distributed.get_rank(), self.config.shared_filesystem)
        ):
            log_single_rank(logger, logging.INFO, f"Build and save the {type(self).__name__} indices")
            self.built_anew_on_cache_miss = True

            # Build the dataset and dataset sample indexes
            log_single_rank(logger, logging.INFO, f"\tBuild and save the dataset and dataset sample indexes")
            t_beg = time.time()
