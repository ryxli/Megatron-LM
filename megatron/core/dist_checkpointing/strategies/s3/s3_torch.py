...
from megatron.core.dist_checkpointing.strategies.s3.s3_async import VirtualWriterAsync
from megatron.core.dist_checkpointing.strategies.s3.s3_filesystem import VirtualReader
from nile_nemo.utils.s3.cloudpath import S3Path
...
class S3TorchDistSaveShardedStrategy(TorchDistSaveShardedStrategy):
    ...
    def async_save(self, sharded_state_dict: Dict[str, Any], checkpoint_dir: Path) -> AsyncRequest:
        ...
        writer = VirtualWriterAsync(checkpoint_dir, thread_count=self.thread_count)
...
class S3TorchDistLoadShardedStrategy(TorchDistLoadShardedStrategy):
    def load(
        ...
        checkpoint_dir: Path | PathBase | str,
    ) -> StateDict:
    ...
    def load_tensors_metadata(self, checkpoint_dir: Path | str, metadata: Metadata = None):
