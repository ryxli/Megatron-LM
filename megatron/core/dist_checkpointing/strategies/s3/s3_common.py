...
class S3TorchCommonSaveStrategy(TorchCommonSaveStrategy):
    def save_common(self, common_state_dict: StateDict, checkpoint_dir: Path | PathBase):
...
class S3TorchCommonLoadStrategy(TorchCommonLoadStrategy):
    def load_common(self, checkpoint_dir: PathBase | os.PathLike | str):
    ...
    def load_sharded_objects(self, sharded_objects_state_dict: ShardedStateDict, checkpoint_dir: Path | PathBase):
        ...
        def load_sharded_object(sh_obj: ShardedObject):
            ...
            try:
                ...
                try:
                    with old_load_path.open(mode="rb") as s3reader:
                        loaded_obj = torch.load(s3reader)
